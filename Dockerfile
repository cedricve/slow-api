FROM mcr.microsoft.com/devcontainers/go:2-1-bookworm AS builder
LABEL AUTHOR=cedricve
...
RUN mkdir -p /go/src/github.com/${REPO_PATH}
COPY . /go/src/github.com/${REPO_PATH}

##################
# Build API

RUN cd /go/src/github.com/${REPO_PATH} && \
    go mod download && \
